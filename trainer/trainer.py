...
from tools.loss import BiDirectionalRankingLoss, WeightTriplet, TripletLoss, NTXent, VICReg, InfoNCE, InfoNCE_VICReg, InfoNCE_new
...
def train(config):
    ...
    elif config.training.loss == 'infonce_new':
            criterion = InfoNCE_new()

    elif config.training.loss == 'infonce+vicreg':
            criterion = InfoNCE_VICReg(info_weight=1,vic_weight=0.4)
  
    else: #config.training.loss == 'bidirect': 'triplet-max'
    ...
    for epoch in range(ep, config.training.epochs + 1):
        ...
        r1, r5, r10, r50, mAP10, medr, meanr = validate(val_loader, model, device, criterion)
        r_sum = r1 + r5 + r10
...
def validate(data_loader, model, device, criterion):
    ...
    with torch.no_grad():
        ...
        r1, r5, r10, r50, mAP10, medr, meanr = t2a(audio_embs, cap_embs)

        val_logger.info('Caption to audio: r1: {:.2f}, r5: {:.2f}, '
                        'r10: {:.2f}, r50: {:.2f}, mAP10: {:.2f}, medr: {:.2f}, meanr: {:.2f}'.format(
                         r1, r5, r10, r50, mAP10, medr, meanr))

        # evaluate audio to text retrieval
        r1_a, r5_a, r10_a, r50_a, mAP10_a, medr_a, meanr_a = a2t(audio_embs, cap_embs)

        val_logger.info('Audio to caption: r1: {:.2f}, r5: {:.2f}, '
                        'r10: {:.2f}, r50: {:.2f}, mAP10: {:.2f}, medr: {:.2f}, meanr: {:.2f}'.format(
                         r1_a, r5_a, r10_a, r50_a, mAP10_a, medr_a, meanr_a))
